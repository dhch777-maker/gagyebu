...
EMPLOYEES = [
    ...
    # 전민아: 2월 부분 월(600,000) → 3~9월 월급 1,800,000 → 10월부터 1,848,000로 인상
    {"name": "전민아", "pay_type": "월급", "amount": 600000,  "account": "하나은행 558-910330-30707",
     "status": "재직", "start_month": 202502, "end_month": 202502},
    {"name": "전민아", "pay_type": "월급", "amount": 1800000, "account": "하나은행 558-910330-30707",
     "status": "재직", "start_month": 202503, "end_month": 202509},
    {"name": "전민아", "pay_type": "월급", "amount": 1848000, "account": "하나은행 558-910330-30707",
     "status": "재직", "start_month": 202510, "end_month": None},
...
# 지급 이력 — xlsb 원본 실지급액 (이름, YYYYMM, 지급여부, 지급액)
# - 이름: 직원 마스터와 일치
# - YYYYMM: 지급 대상 월 (xlsb 지급일 정규화: day≤10=전월분, day>10=당월분)
# - 지급여부: 현재까지 모두 지급 완료(전부 "O")
# - 지급액: xlsb 실지급액. 급여 요약 총급여는 이 값을 우선 사용 (없으면 마스터 기반 계산)
# 2025-12는 xlsb 미기록분이므로 패턴 기반 예측값 (사용자 확인 후 수정)
PAYMENT_HISTORY = [
    # 2025-01
    ("고경민", 202501, "O", 1200000),
    ("유은비", 202501, "O", 528000),
    ("장예원", 202501, "O", 786000),
    # 2025-02
    ("고경민", 202502, "O", 1200000),
    ("유은비", 202502, "O", 240000),
    ("장예원", 202502, "O", 744000),
    ("전민아", 202502, "O", 600000),
    # 2025-03
    ("고경민", 202503, "O", 912000),
    ("유은비", 202503, "O", 240000),  # xlsb 누락분. 근무 20h·시급환산·퇴사월
    ("장예원", 202503, "O", 768000),
    ("전민아", 202503, "O", 1800000),
    # 2025-04
    ("고경민", 202504, "O", 792000),
    ("장예원", 202504, "O", 708000),
    ("전민아", 202504, "O", 1800000),
    # 2025-05
    ("고경민", 202505, "O", 1008000),
    ("장예원", 202505, "O", 516000),
    ("전민아", 202505, "O", 1800000),
    # 2025-06
    ("고경민", 202506, "O", 600000),
    ("김채현", 202506, "O", 120000),
    ("장예원", 202506, "O", 792000),
    ("전민아", 202506, "O", 1800000),
    # 2025-07
    ("고경민", 202507, "O", 672000),
    ("김채현", 202507, "O", 660000),
    ("장예원", 202507, "O", 624000),  # xlsb 누락분. 근무 52h·시급환산·퇴사월
    ("전민아", 202507, "O", 1800000),
    # 2025-08
    ("고경민", 202508, "O", 792000),
    ("김채현", 202508, "O", 648000),
    ("전민아", 202508, "O", 1774000),  # 결근·상여 등 조정으로 -26,000
    # 2025-09
    ("고경민", 202509, "O", 588000),
    ("김채현", 202509, "O", 534000),
    ("전민아", 202509, "O", 1800000),
    # 2025-10
    ("고경민", 202510, "O", 1300000),
    ("김채현", 202510, "O", 438000),  # xlsb 누락분. 근무 36.5h·시급환산·퇴사월
    ("이진화", 202510, "O", 480000),
    ("전민아", 202510, "O", 1848000),
    # 2025-11
    ("고경민", 202511, "O", 1300000),
    ("이진화", 202511, "O", 480000),
    ("전민아", 202511, "O", 1848000),
    # 2025-12 — xlsb 미기록분, 패턴 예측 (필요 시 실제 값으로 수정)
    ("고경민", 202512, "O", 1300000),
    ("이진화", 202512, "O", 348000),
    ("전민아", 202512, "O", 1848000),
]
...
def build_payment_history_sheet(wb):
    ...
    for idx, (name, ym, paid, amount) in enumerate(PAYMENT_HISTORY):
        r = idx + 2
...
def build_summary_sheet(wb):
    ...
    for idx, emp in enumerate(EMPLOYEES):
        ...
        ws.cell(row=r, column=5).value = (
            f'=IF(B{r}="","",IFERROR({paid_lookup},{fallback}))'
        )
